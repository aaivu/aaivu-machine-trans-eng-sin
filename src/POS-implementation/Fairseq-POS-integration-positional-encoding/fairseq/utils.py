...
def concat_embeddings(embedding_1, embedding_2):
  ...
  embedding_concat = torch.nn.Embedding.from_pretrained(embedded_weight)
...
def merge_embeddings(embedding_1, embedding_2, mapping):
  ...
  final_embedding = torch.nn.Embedding.from_pretrained(cat_weights, padding_idx=embedding_1.padding_idx)
  return final_embedding

def mapping_to_embedding(embedding_1, mapping, embedding_2):
  weights = []
  for i in range(len(mapping)): 
    input_1 = torch.LongTensor([mapping[i]])
    weights.append(embedding_1(input_1))

  cat_weights = torch.cat(weights, dim=0)
  final_embedding = torch.nn.Embedding.from_pretrained(cat_weights, padding_idx=embedding_2.padding_idx)
  return final_embedding
