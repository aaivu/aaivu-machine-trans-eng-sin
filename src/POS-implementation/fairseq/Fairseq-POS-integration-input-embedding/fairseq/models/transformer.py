...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--encoder-num-pos', type=int, metavar='N',
                            help='path to pos tag mapping')
        parser.add_argument('--encoder-pos-embed-dim', type=int, metavar='N',
                            help='POS tag dimension')  

        parser.add_argument('--combine-bpe-path', type=str, metavar='STR',
                            help='path to combine-bpe-en-file')
        parser.add_argument('--combine-numbers-path', type=str, metavar='STR',
                            help='path to combine-numbers-file')
    ...
    def build_model(cls, args, task):
        ...
        if args.share_all_embeddings:
            ...
            if args.pos_embed_path:
                raise ValueError(
                    "--share-all-embeddings not compatible with --posr-embed-path"
                )
            ...
            if args.encoder_num_pos != 0: 
              if args.encoder_pos_embed_dim == 0:
                raise ValueError("--encoder-pos-embed-dim should be higher than zero ( > 0) if --encoder_pos_embed_path is enabled")
              encoder_embed_tokens = cls.build_embedding(
                args, src_dict, args.encoder_embed_dim - args.encoder_pos_embed_dim,
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.combine_bpe_path = args.combine_bpe_path
        self.combine_numbers_path = args.combine_numbers_path
    ...
    def load_train(self):
        filename1 = self.combine_bpe_path #tagged file name (word:tag)        
    ...
    def load_pos_tag(self):
        filename1 = self.combine_numbers_path
        file1 = open(filename1, encoding="utf8").readlines() 
        pos_tag = [] 
        for lines in file1:
    ...
    def forward_embedding(self, src_tokens):
        ...
        l = src_tokens.tolist()                      
        tagList = self.processBatch(l) 
        
        if self.pos_tokens!=0:
	        	x_pos = self.pos_tokens(tagList)
...
def base_architecture(args):
    ...
    args.encoder_num_pos = getattr(args, "encoder_num_pos", 0)	
    args.encoder_pos_embed_dim = getattr(args, "encoder_pos_embed_dim", 0)

    args.combine_bpe_path = getattr(args, "combine_bpe_path", None) 
    args.combine_numbers_path = getattr(args, "combine_numbers_path", None) 
